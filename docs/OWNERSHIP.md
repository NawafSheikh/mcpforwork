...
# Wave 4 ownership, turns and wiring (29 Aug 2026)

| Owner | Paths | Delivers |
|---|---|---|
| A16 turns+wiring | src/turns/** (new), src/shell/RequestsButton.tsx, src/shell/LockedRoom.tsx, this row block, plus the wiring edits listed below | The turn model of docs/TURNS.md: `Claim` and `WriteMark` on the Workspace, the `claim`, `release` and `list_claims` tools (28 in the contract), automatic claims on every write, field-level merge of two agents' writes instead of a refusal, `expectedUpdatedAt` on the three replacing writes, claim badges on dashboard and monitor cards and in presence. Also the wiring of A15's four-way notes (widened share coercers, `RoomRequests` in a header popover, `NameChip` feeding the room presence label) and of A17's encryption (`sealedTransport`, invite links carrying the key, the locked-room card, the store key including the key fingerprint). Wiring edits outside src/turns: src/types.ts, src/store/{createStore,selectors}.ts, src/share/{caps,ops,snapshot}.ts, src/rooms/**, src/webmcp/**, src/monitors/handlers.ts, src/shell/{Header.tsx,adapters/monitors.ts,adapters/webmcp.ts,tabs/MonitorCard.tsx,tabs/AboutTab.tsx,tabs/board/{CategoryPane,mutate,useBoardEdits}}, src/main.tsx, src/demo/sampleWorkspace.ts, src/styles/app.css. |

# Wave 5 ownership, packs and capabilities (29 Aug 2026)

| Owner | Paths | Delivers |
|---|---|---|
| A20 packs+capabilities | src/packs/** (new), src/capabilities/** (new), src/webmcp/{register,registry,definitions,schemas,jsonSchemas,annotations}.ts (pack gate and the two-tool merge), src/types.ts (`packs` and `capabilities` on Workspace, both optional), src/store/createStore.ts (two coerce lines), src/rooms/{types,diff,apply}.ts (the `pack` and `capability` patch kinds), the "## Packs" section of docs/TOOLS.md, this row block | docs/PACKS.md made real: the 30 tools grouped into six built-in packs (`board`, `datasets`, `notes`, `turns`, `monitors`, `rooms`) with a risk each, all on by default and `send`/`move` off in rooms. `registerAllTools` is pack-aware, one AbortController per pack, so a switch takes exactly its tools off `document.modelContext` at once; the registry refuses a call to a switched-off tool with "The &lt;pack&gt; pack is off in this room; ask the host." and audits it `ok: false`. In a room only the host may switch (rooms exposes no host, so the fallback is the lowest client id, which every browser computes identically); switches sync as the `pack` entity. `src/packs/bridge.ts` speaks the mcpforwork-bridge wire protocol over an injectable socket (never auto-connects), verifies the Ed25519 hello where WebCrypto can, refuses a `move` pack without a stop and a boundary, turns every bridge pack into a page pack, and unregisters everything on disconnect; robot profiles and queue events surface through a small event bus. `PacksPanel` (switches, risk chips, tool counts, who changed it, Local bridge, work-pack catalog) and `CapabilityCard` / `CapabilityCards` are exported for A19 to mount; `publish_capabilities` and `list_capabilities` are the two new tools, in the `rooms` pack. Mount points in src/packs/INTEGRATION.md. **Wiring left to the shell**: spread `capabilityHandlers` into `handlersFor()` in src/shell/adapters/webmcp.ts, pass `createPackController(store)` as `packs` to both `createToolRegistry` and `registerAllTools`, and add `connect-src ws://127.0.0.1:* ws://localhost:*` to the CSP in index.html and vercel.json. |
